...
idioam_lang(){
  source <(curl -sSl 'https://raw.githubusercontent.com/adiel-dev/ramirez/main/lang')
  ...
  for arch in $listarq; do
    if ! wget -O $lang_dir/${list_lang[$lg]}/$arch "https://raw.githubusercontent.com/adiel-dev/ramirez/main/lang/${list_lang[$lg]}/$arch" &>/dev/null ;then
...
if [[ ! -e $udp_file/UDPserver.sh ]]; then
  mkdir $udp_file
  chmod -R +x $udp_file
  source <(curl -sSL 'https://raw.githubusercontent.com/adiel-dev/ramirez/main/module')
  idioam_lang
  [[ -e $lang ]] && newlang=$(cat $lang) && [[ ! $newlang = '' ]] && source $udp_file/lang/$newlang/UDPserver
  source /etc/os-release
  check_sistem
	wget -O $udp_file/module 'https://raw.githubusercontent.com/adiel-dev/ramirez/main/module' &>/dev/null
	chmod +x $udp_file/module
	#source $udp_file/module
	wget -O $udp_file/limitador.sh "https://raw.githubusercontent.com/adiel-dev/ramirez/main/limitador.sh" &>/dev/null
...
download_udpServer(){
	...
	if git clone -O /usr/bin/udpServer 'https://github.com/adiel-dev/ramirez/raw/main/udpServer' &>/dev/null ; then
